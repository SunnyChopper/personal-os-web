...
export default function HealthFitnessNutritionPage() {
  ...
  const { data, isLoading } = useFitnessNutritionList({ startDate: start, endDate: end, pageSize: 30 });
    ...
    <div className="mx-auto max-w-3xl space-y-8 p-6">
      <div>
        ...
        <p className="mt-2 text-gray-600 dark:text-gray-400">
          Parse free text with <code className="rounded bg-gray-100 px-1 dark:bg-gray-800">POST /ai/fitness/nutrition/parse</code>, edit, then save to{' '}
          ...
          {rows.map((e) => (
            ...
            >
              ...
              <span className="font-mono text-xs text-gray-600 dark:text-gray-400">
                {Math.round(e.calories)} kcal · P{e.proteinGrams.toFixed(0)} C{e.carbGrams.toFixed(0)} F
                {e.fatGrams.toFixed(0)}
              </span>
            </li>
          ))}
        </ul>
